...
def load_image_path(
    ...
    arr = center_crop_square(arr, crop_size)
    if resize:
...
def center_crop_square(arr: np.ndarray, crop_size: int | None = None) -> np.ndarray:
    h, w = arr.shape[:2]
    side = min(h, w) if crop_size is None else min(crop_size, h, w)
    y0 = (h - side) // 2
    x0 = (w - side) // 2
